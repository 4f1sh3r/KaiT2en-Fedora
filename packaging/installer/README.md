...
3. Boot the T2 Mac from that USB drive and install Fedora normally. Format an
   ...
   archive to a second USB drive. The `Rescue: repair an installed KaiT2en
   system` boot entry starts a text console with
   `sudo /run/kait2en/kait2en-live-rescue`, which mounts an already installed
   Fedora and chroots into it.
