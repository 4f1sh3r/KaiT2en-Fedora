...
pub fn connect_remote_service(
    ...
    if let Some(window) = initial_window {
        connection.start_with_window(window)?;
    } else {
        connection.start()?;
    }
    connection.frame(HEADERS, 4, 1, &[])?;
    ...
    let received = connection
        .stream
        .read(&mut scratch)
        .context("read remote service initial handshake")?;
    ensure!(received != 0, "remote service closed during initial handshake");
...
impl RemoteService {
    pub fn send(&mut self, value: &Value, flags: u32, id: u64) -> Result<()> {
        self.connection.frame(DATA, 0, 1, &xpc::wrapper(Some(value), flags, id))
    }
