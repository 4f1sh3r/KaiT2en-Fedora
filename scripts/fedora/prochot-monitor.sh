...
while true; do
    ...
    echo "ctrl+c to quit"
    echo
    printf "%-6s %-8s %-8s %-8s %-11s %-6s %-26s\n" "Core" "MHz" "Temp" "PROCHOT" "Log(sticky)" "dTj" "Status"
    echo "-----------------------------------------------------------------------"

    ANY_ACTIVE=0

    for i in $(seq 0 $((NCPU - 1))); do
        VAL_HEX=$(rdmsr -p "$i" "$MSR_THERM_STATUS" 2>/dev/null)
        if [ -z "$VAL_HEX" ]; then
            printf "%-6s %-8s %-8s %-8s %-11s %-6s %-26s\n" "$i" "?" "?" "?" "?" "?" "read error"
            continue
        fi

        TJMAX_HEX=$(rdmsr -p "$i" "$MSR_TEMP_TARGET" 2>/dev/null)
        if [ -n "$TJMAX_HEX" ]; then
            TJMAX=$(( (16#$TJMAX_HEX >> 16) & 0xFF ))
        else
            TJMAX=100
        fi
        ...
        BIT2=$(( (VAL_DEC >> 2) & 1 ))   # PROCHOT#/FORCEPR# event, currently active (internally OR externally)
        ...
        printf "%-6s %-8s %-8s %-8s %-11s %-6s %-26s\n" "$i" "${MHZ:-?}" "${TEMP}°C" "$BIT2" "$BIT3" "$READOUT" "$STATUS"
    done
