...
COPR imports uploaded SRPMs into its Dist-Git before assigning a builder. If
that internal download fails with a truncated response, the workflow retries
the same SRPM up to two times. Patch, configuration, and compiler failures are
not retried.
